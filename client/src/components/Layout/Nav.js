        ...
        <Box as='li'>
          {isDesktop &&
            <Menu>
              ...
              <MenuList padding={0} boxShadow='0 10px 15px -3px rgb(0 0 0 / 0.2), 0 4px 6px -4px rgb(0 0 0 / 0.2)'>
                <MenuItem as={NavLink} to={`/accounts/${authCtx.user.username}`} display='flex' gap={1} alignItems='center' _hover={{ textDecoration: 'none' }}><IoMdPerson size='20px' />Profile</MenuItem>
                <MenuItem onClick={toggleColorMode} display='flex' gap={1} alignItems='center'><ColorModeSwitcher size='20px' fontSize='lg' />Switch Color Mode</MenuItem>
                <MenuItem onClick={logoutHandler} display='flex' gap={1} alignItems='center'><IoMdLogOut size='20px' />Log Out</MenuItem>
              </MenuList>
            </Menu>}
          {!isDesktop &&
